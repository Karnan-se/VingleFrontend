...
export default function StudentTable() {
    ...
    const fetchcoursePurchased = async () => {
      try {
        ...
        students.forEach((student) => {
          const matchingOrder = Orders.filter(
            (order) =>
              order.userId?._id == student?._id &&
              order.paymentStatus == "Completed"
          );
          const coursePurchased = matchingOrder
            .map((order) => order?.courseId?.name)
            .filter(Boolean);
        

          student.purchasedCourse = coursePurchased.length
            ? coursePurchased
            : [];
        });
  ...
  const filteredStudents = students.filter((student) => {
    ...
    const matchesStatus =
      statusFilter === "all" || student.isBlocked == (statusFilter == "true");
                      ...
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10">
                          <img
                            ...
                            src={student.photo || "https://www.shutterstock.com/image-vector/blank-avatar-photo-place-holder-600nw-1114445501.jpg"}
                            alt=""
                            width={40}
                            height={40}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-2">
                        {student.purchasedCourse &&
                          student.purchasedCourse.map(
                            (purchasedCourse, index) => (
                              <span
                                key={index}
