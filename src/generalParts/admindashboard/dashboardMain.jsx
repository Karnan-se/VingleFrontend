...
const transformRevenueData = (data, period) => {
  ...
  return Object.values(transformedData).sort((a, b) => new Date(a.date) - new Date(b.date))
}
    ...
    <>
      <main className="flex-1 p-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow mb-8">
          <Chart data={data} activeRange={activeRange} timeRanges={timeRanges} handleRangeChange={handleRangeChange} />
        </div>

        {/* Bottom row with two cards */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-gray-500 text-sm">Draw Usage Count</h3>
            <p className="text-2xl font-semibold mt-2">
              125 <span className="text-sm text-gray-500">/ month</span>
            </p>
            <p className="text-sm text-gray-500 mt-1">
              85 (previous month) <span className="text-green-500">+47%</span>
            </p>
          </div>
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-gray-500 text-sm">Account Deletions</h3>
            <p className="text-2xl font-semibold mt-2">
              10 <span className="text-sm text-gray-500">/ month</span>
            </p>
            <p className="text-sm text-gray-500 mt-1">
              8 (previous month) <span className="text-green-500">+25%</span>
            </p>
          </div>
        </div>
      </main>
