  ...
  useEffect(() => {
    async function getTotalCourse() {
      try {
        ...
        console.log(tutorId.emailAddress, "emailAddress");

        const response = await adminApi.get("/getallStudents");
        const allUserDetails = response.data.students;
        console.log(allUserDetails, "allUser");

        const stud = allUserDetails.filter(
          (student) => student.emailAddress == tutorId.emailAddress
        );
     

        const [totalCourseResult, instructorDetailsResult] = await Promise.all([
          totalCourse(tutorId._id),
          getInStructorDetails(stud[0]._id),
        ]);
